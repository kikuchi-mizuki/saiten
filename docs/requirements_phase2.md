...
### 開発期間
**8週間**（Week 1-2: RAG強化 → Week 3-4: ナレッジベース管理 → Week 5-6: 音声入力 → Week 7: PPT生成 → Week 8: テスト）
...
```
【ナレッジベース構築フロー】
教授 → 音声録音/テキスト入力 → Whisper API（音声の場合）
...
**機能**:
- ブラウザ標準APIで音声録音（MediaRecorder API）
- 録音時間の表示（上限10分）
- Whisper APIでテキスト変換
- 変換結果の編集
- LLMによる自動タグ付け + 手動編集

#### 3.2 バックエンドAPI（POST /upload-audio）

**入力**:
- audio_file: 音声ファイル（.mp3, .wav, .m4a）
- user_id: 認証ユーザーID

**処理**:
1. Whisper APIで音声をテキスト化
2. LLM（gpt-4o-mini）で自動タグ付け
3. 提案タグを返却

**出力**:
- transcribed_text: テキスト化された内容
...
| Week 5-6 | 音声入力機能 | 音声録音、Whisper API統合、自動タグ付け |
...
6. バックエンドAPI実装
   - POST /upload-audio（音声アップロード）
   - POST /transcribe（テキスト変換）
