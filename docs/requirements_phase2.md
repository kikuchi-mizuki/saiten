...
| バージョン | 日付 | 変更内容 | 作成者 |
|----------|------|---------|--------|
| 1.1 | 2025-12-03 | 最終レポート用要約フォーマット変更（目指す姿、本質的課題、具体的施策、実践課題の観点で要約） | Claude |
...
### 2025-12-03: 最終レポート用要約フォーマット変更

**実装内容**:
- 最終レポート（`doc_type="final"`）の要約を以下の観点で生成するように変更：
  1. 自分が関わっている事業・組織の目指す姿
  2. 本質的課題：めざす姿と現状のギャップ
  3. ギャップを埋めるための具体的施策（講義内容の応用、時間軸）
  4. 実践するにあたっての課題とその解決策の仮説

**変更ファイル**:
- `api/main.py`
  - `generate_summary()`に`doc_type`パラメータを追加
  - `generate_summary_llm()`に`doc_type`パラメータを追加
  - 最終レポート用の要約プロンプトを新規作成
  - `generate_direct`エンドポイントで`doc_type`を渡すように修正

**技術的詳細**:
- 振り返りレポート（`doc_type="reflection"`）は従来通りの要約形式を維持
- LLM（gpt-4o-mini）を使用して、指定された観点で構造化された要約を生成
- 要約の出力形式は絵文字付きセクション（1️⃣, 2️⃣, 3️⃣, 4️⃣）で統一
