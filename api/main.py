...
def generate_summary_llm(text: str, doc_type: str = "reflection") -> Tuple[Dict[str, any], bool]:
	...
	try:
		...
		if doc_type == "final":
			# 最終レポート用：指定された観点で要約
			summary_prompt = f"""以下のレポート本文を読んで、記載されている内容を忠実に要約してください。

【レポート本文】
{text}

【要約の作成方針】
- レポートに記載されている内容のみを基に要約を作成してください
- 推測や一般論を追加せず、レポートの具体的な内容をそのまま反映してください
- 企業名、事業名、戦略名、専門用語などはレポートに記載されている通りに使用してください
- 以下の観点で要約を構成してください：
...
要約を出力してください:"""
		...
		summary_text, error = call_openai_summary(
			summary_prompt,
			system_prompt="あなたは学術的な要約を作成する専門家です。レポートに記載されている内容を忠実に要約し、推測や一般化を避けてください。企業名、事業名、戦略、専門用語などはレポートに記載されている通りに使用し、レポートの具体的な内容を正確に反映した要約を作成してください。"
		)
